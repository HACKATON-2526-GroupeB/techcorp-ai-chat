...
<template>
  <aside
    class="fixed left-0 top-0 h-full w-[280px] flex flex-col z-20 border-r border-white/5"
    style="background:#101018"
    @click="closeMenu"
  >
    <!-- Logo -->
    <div class="px-5 pt-5 pb-4 flex items-center gap-2.5">
      <div class="w-8 h-8 rounded-lg flex items-center justify-center" style="background:linear-gradient(135deg,#7C5CFC,#5A4FCF)">
        <span class="material-symbols-outlined text-white text-[18px]">auto_awesome</span>
      </div>
      <span class="font-bold text-white text-[15px] tracking-tight">TechCorp <span style="color:#7C5CFC">AI</span></span>
    </div>

    <!-- New Chat -->
    <div class="px-3 mb-4">
      <button
        class="w-full flex items-center gap-2 justify-center py-2.5 rounded-xl text-sm font-semibold text-white transition"
    ...
    <div class="px-3 mb-3">
      <div class="flex items-center gap-2 px-3 py-2 rounded-xl border border-white/8" style="background:#1e1e2e">
        ...
        <input
          ...
          class="flex-1 bg-transparent text-xs text-white placeholder-white/30 outline-none"
        />
      </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto px-3 pb-3">
      ...
      <div
        ...
        :class="s.id === currentSessionId
          ? 'border border-[#7C5CFC]/30 text-white'
          : 'hover:bg-white/5 text-white/60 hover:text-white'"
        :style="s.id === currentSessionId ? 'background:rgba(124,92,252,0.12)' : ''"
        @click="loadSession(s)"
      >
        <span class="material-symbols-outlined text-[16px] flex-shrink-0" :class="s.id === currentSessionId ? 'text-[#7C5CFC]' : 'text-white/30'">chat_bubble_outline</span>
        <div class="flex-1 min-w-0">
          <p class="text-xs font-medium truncate">{{ s.title }}</p>
          <p class="text-[10px] text-white/30 mt-0.5">{{ relativeTime(s.ts) }}</p>
        </div>
        <!-- Context menu trigger -->
        <button
          class="opacity-0 group-hover:opacity-100 p-1 rounded-lg hover:bg-white/10 text-white/40 hover:text-white transition flex-shrink-0"
      ...
      <p v-else-if="!filtered.length && search" class="text-xs text-white/25 px-2 py-4 text-center">Aucun résultat</p>
    </div>
